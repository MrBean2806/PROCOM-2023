...
def funcion_trama(data):
    # Arma el vector a transmitir


    # Cabecera

    #   Inicio de trama. Bits 0 a 2
    inicio_de_trama = [1, 0, 1]
    for ptr in range(len(inicio_de_trama)):
        char_v.append(str(inicio_de_trama[ptr]))

    #   L:1/S:0 (L: Trama larga/ S: Trama corta). Bit 3
    L_S = 0
    char_v.append(str(L_S))

    #   S. Size. Bit 4 a 7
    #   Obtengo la longitud de data (int).
    aux = len(data) 
    #   Convierto el valor entero decimal, a representación binaria (tipo String).
    S_size = format(aux, '04b') 
    #   Transmito los bits, iniciando por el bit más significativo. 
    for ptr in range(4):
        char_v.append(S_size[ptr])

    #   L. Size
    #   L. Size (High). Bits 8 a 15
    L_size_high = [0] * 8
    for ptr in range(len(L_size_high)):
        char_v.append(str(L_size_high[ptr])) 
    #   L. Size (Low). Bits 16 a 23  
    L_size_low = [0] * 8
    for ptr in range(len(L_size_low)):
        char_v.append(str(L_size_low[ptr])) 

    #   Device. Bits 24 a 31    
    device = '0' * 8
    for ptr in range(len(device)):
        char_v.append(device[ptr]) 
    ...
    # Fin de trama
    final_de_trama = [0, 1, 0]
    for ptr in range(len(final_de_trama)):
        char_v.append(str(final_de_trama[ptr]))
...
def verificar_trama(out):
    out_data = ''

    #Verifico si la trama es correcta
        # '101' == out[0:3]. Verifico el 101 inicial
        # out[3] == out[-5]. Verifico L/S.
        # out[4:8] == (out[-4:-1]+out[-1]). Verifico S.size. Lo opero de esa manera porque llega a 1 antes del out[-1].
        # out[24:31] == '00000000'. Verifico Device.
        # '010' == out[-8:-5]. Verifico el 010 de los 8 bits finales.
    ...
    print(out[-5])
    print(out[-8:-5])'''
    if '101' == out[0:3] and out[3] == out[-5] and out[4:8] == (out[-4:-1]+out[-1]) and out[24:32] == '00000000' and '010' == out[-8:-5]:

        # Caso S.size
        if out[3] == '0' and out[-5] == '0':
            # Verifico en S.size la cantidad de bytes recibidos en Data.
            S_size_recibido = 0
            for ptr in range(-4, 0, 1):
                # Tomo los últimos cuatro valores, y los multiplico por 2 a la n según corresponda, para obtener el S.Size en entero.
                S_size_recibido = S_size_recibido + int(out[ptr])*(2**(-ptr - 1))

            
            for prt in range(32, 32 + S_size_recibido, 1):
                out_data += out[prt]

        # Caso L.size
        elif out[3] == '1' and out[-5] == '1':
            # Verifico en S.size la cantidad de bytes recibidos en Data.
            L_size_recibido = 0
            for ptr in range(8, 24, 1):
                # Similar al caso S.size.
                L_size_recibido = L_size_recibido + int(out[ptr])*(2**(-ptr + 23))

            
            for prt in range(32, 32 + L_size_recibido, 1):
                out_data += out[prt]
...
while 1 :
    char_v = []
    data_a_enviar = input('''Ingrese un comando:
    -Calculadora: Ejecuta el script calculadora.
    -Graficar: Ejecuta el script de gráficos.
    -exit: Finaliza el programa.
Respuesta: ''')
    
    # Llamo a la función que arma la trama
    char_v = funcion_trama(data_a_enviar)

    # print(char_v)
    

    # Llamo a la función que envía por puerto serie
    enviar_trama(ser, char_v)

    # Llamo a la función que recibo por puerto serie
    out = recibir_trama(ser)

    # Llamo a la función que verifica si la trama recibida es correcta. Esta función además envía la Data
    out_data = verificar_trama(out)
    ...
    if out_data != '':
        print(">> " + out_data)
        if out_data == 'Calculadora':
            script_calculadora()
        elif out_data == 'Graficar':
            graficar()
        elif out_data == 'exit':
            print('Comando de finalización recibido, se termina el script.')
            if ser.isOpen():
                ser.close()
            break
    print('\n')
